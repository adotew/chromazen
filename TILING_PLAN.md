...
The existing layer-display pipeline now accepts a 16-byte document-origin/extent uniform and samples only that region; the full-document binding uses origin zero and its former dimensions (including after resize). A GPU regression renders a 7 × 13 source at document position [25, 30] and verifies every displayed pixel and transparent surroundings. Another draws three sparse 32 × 32 textures (one quadrant missing) through that same pipeline and compares all 80 × 80 output pixels exactly against a dense source under fractional zoom, rotation and horizontal flip. The current paint sampler uses nearest filtering, so these draws require no bilinear halo. This changes the **existing** pipeline, not a parallel renderer. Layers still own full-document textures; clipped groups, previews and thumbnails have not yet been converted to tile sampling.
...
Transform content-bounds selection consumes mapped readback rows without a decoded CPU layer. It now reads full-width bands of **at most 8 MiB padded GPU staging each**, reserving them against the shared 16 MiB readback budget. It caches bounds only after every band succeeds; a GPU regression exercises budget failure and retry, bottom/right alpha, clear, and undo. This still scans the whole image **synchronously** through its existing whole-document layer texture; sparse per-tile bounds metadata and incremental transform readiness remain necessary in phase 3.
...
Native autosave now reuses both unchanged whole-layer PNGs **and the previous thumbnail without any GPU readback** when a save changes only nonvisual metadata (title, selection, brush color, references or layer names). A visual signature tracks saved background and ordered layer visibility/opacity/clipping across in-flight revisions; a visually changed thumbnail still uses the existing full-layer readback and encoding. Thumbnail reuse is copied/hard-linked inside the new temporary revision before atomic publication, with a first-revision guard and store/metadata tests. Pending explicit saves now report `Waiting` until started so navigation cannot mistake a title-only rename for a clean session. This avoids work for metadata-only saves; it does **not** bound pixel-changing autosave.
