...
Native export now composites premultiplied RGBA8 one row at a time into a directly streamed PNG writer inside `AtomicWriteFile`, instead of allocating a second full-size composite and a complete encoded PNG alongside the layer images. It uses the same validated row compositor as the whole-image raster adapter, with byte-for-byte decoded pixel tests for odd-width multi-row clipped/opacity layers; invalid input preserves the previous export. `png = 0.18.1` is now a direct root dependency. **Export is not yet bounded end-to-end**: `Canvas::begin_document_layer_readback` and `LayerReadback::finish` still stage/decode complete layers, and row source access must be replaced with a versioned tile/band snapshot before lifting document limits. Browser artwork export is unchanged.
