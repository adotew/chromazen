...
function dispatch(command) {
	...
	return invoke("dispatch", { command }).catch(console.error);
}
...
function renderLayers(layers) {
	...
	const entries = [...layers.layers].reverse().map((layer) => ({
		label: layer.name,
...
elements.addLayer.addEventListener(
	"click",
	() => void dispatch({ type: "addLayer" }),
);
elements.deleteLayer.addEventListener(
	"click",
