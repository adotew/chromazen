...
- `T`: select Transform. The frame fits the painted pixels on the selected
  layer. Drag inside it to move, drag its square handles to scale uniformly, or
  drag the circular handle to rotate. Press `Enter` to bake the transform or
  `Escape` to restore the original pixels. Applied transforms are undoable.
